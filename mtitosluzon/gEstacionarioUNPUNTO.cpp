...
#define ficheroResultados "resultados/resultadosGeneticoEstacionarioUNPUNTO.txt"
#define ficheroResultados2 "resultados/resultadosGEUNPUNTO_evoluciones.csv"
...
int main(int args, char* argv[])
{
	if( args == 2 )
	{
		...
		for(unsigned int i = 0; i < N_SEMILLAS; i++)
		{
			...
			for(unsigned int ind = 0; ind < POBLACIONGENETICOS; ind++) // me creo mi población de cromosomas
			{
			...
			{
				/*for(unsigned int l=0;l<poblacion.size();l++){
					if(poblacion[l].size()!=datosentrenamiento[0].size()-1)
						cout << "Cromosoma erroneo en: " << l<< "en la pasada: "<< contador<<endl;
				}
				contador++;*/	
