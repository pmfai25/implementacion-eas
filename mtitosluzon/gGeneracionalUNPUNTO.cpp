/**
 * @file gGeneracional.cpp Fichero de implementación del método del algoritmo genético generacional
...
#define ficheroResultados "resultados/resultadosGeneticoGeneracionalUNPUNTO.txt"
#define ficheroResultados2 "resultados/resultadosGGUNPUNTO_evoluciones.csv"
...
int main(int args, char* argv[])
{
	if( args == 2 )
	{
		...
		for(unsigned int i = 0; i < N_SEMILLAS; i++)
		{
			...
			for(unsigned int j = 0; j < POBLACIONGENETICOS; j++) // me creo mi población de cromosomas
			{
				...
				individuoNuevo.iniciarCromosoma(datosentrenamiento[0].size()-1, datosentrenamiento, datostest, clasesentrenamiento, clasestest);
			...
			for(j = 0; j < EVALUACIONES; j++)
			{
				...
				unsigned int posicionPadre, posicionMadre;
				cromosoma padre, madre, descendiente1, descendiente2;
				//cout << "\tINFORMACION: comienza el torneo generacional" << endl;
				seleccionGeneracional(padre, madre, posicionPadre, posicionMadre, poblacion, datosentrenamiento, datostest, clasesentrenamiento, clasestest);
				if( Randfloat(0, 1) <= PROBABILIDADCRUCEGENERACIONAL )
				{
				//	cout << "\tINFORMACION: cruzo" << endl;
					...
					cruceUnPunto(padre, madre, descendiente1, descendiente2, datosentrenamiento, datostest, clasesentrenamiento, clasestest);
					//j += 2;
				}
				cout << "\tINFORMACION: reemplazo" << endl;
				reemplazarCromosomasGeneracional(descendiente1, descendiente2, posicionPadre, posicionMadre, poblacion, datosentrenamiento, datostest, clasesentrenamiento, clasestest);
				// Muto la población
				for(unsigned int k = 0; k < poblacion.size(); k++)
				{
					// Probablidad de mutar
					if( Randfloat(0, 1) <= PROBABLIDADMUTARGENERACIONAL )
					{
		...
		//guardarResultadosFichero(argv[1], ficheroResultados, datosfinalesentrenamiento, datosfinalestest, datosfinalesreduccion,solucionesfinales);
	...
	else
	{
		...
		cerr << "\t\tbin/gGeneracional ozone" << endl;
		cerr << "\t\tbin/gGeneracional sonar" << endl;
		cerr << "\t\tbin/gGeneracional spambase" << endl;
	}
}
